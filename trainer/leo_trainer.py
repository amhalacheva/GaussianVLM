...
from model.gs_agent import GSAgent
...
class LeoTrainer():
    def __init__(self, cfg):
        ...
        for mode, loaders in self.data_loaders.items():
            for task, loader in loaders.items():
                ...
                # Note to self: up to here - correct
        accelerate_loaders = self.accelerator.prepare(*all_loaders)
        if len(accelerate_loaders) >20:
            for k in all_loader_keys:
                self.data_loaders[k[0]][k[1]] =accelerate_loaders
    ...
    def run(self):
        if self.mode == 'train':
            ...
            best_pth_path = os.path.join(self.exp_dir, 'best.pth')
            if not os.path.exists(best_pth_path):
                is_best = self.val_step(self.epochs)
                if is_best:
                    self.save('best.pth', model_only=True)   # save the best checkpoint
                    self.accelerator.wait_for_everyone()
            ...
            self.save('best_last.pth', model_only=True)   # save the best checkpoint
            self.accelerator.wait_for_everyone()
            self.load(best_pth_path, model_only=True)
