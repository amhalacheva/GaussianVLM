...
from transformers import AutoModelForCausalLM, AutoTokenizer, LlamaForCausalLM, LlamaTokenizer, PreTrainedTokenizerFast
...
class LeoAgent(nn.Module):
    def __init__(self, cfg):
        ...
        if 'vicuna' in cfg.llm.name.lower():
            ...
            self.llm_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3-8B")
            self.llm_model = AutoModelForCausalLM.from_pretrained("meta-llama/Meta-Llama-3-8B", torch_dtype=torch.float16)
    ...
    def build_right_justified_sequence(self, data_dict):
        ...
        # TODO obj_tokens = data_dict['obj_tokens'].to(device)
        ...
        inputs_embeds_mid = torch.cat([inputs_embeds_mid1, img_tokens, inputs_embeds_mid2], dim=1) # TODO , obj_tokens], dim=1)
