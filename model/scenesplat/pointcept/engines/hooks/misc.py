        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                if comm.get_world_size() == 1:  # Remove prefix if not DDP
                    processed_key = processed_key[7:] if processed_key.startswith("module.") else processed_key
            ...
            if self.strict:
                self.trainer.logger.info(f"Strict mode: Missing keys: {load_state_info[0]}")
                self.trainer.logger.info(f"Strict mode: Unexpected keys: {load_state_info[1]}")
...
@HOOKS.register_module()
class BeginningEvaluator(HookBase): # for debugging
    def __init__(self, test_last=False):
        self.test_last = test_last
