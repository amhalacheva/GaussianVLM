...
class Point(Dict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "batch" not in self.keys():
            self.batch = torch.zeros(self.coord.shape[0], dtype=torch.int, device='cuda')
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        if "grid_coord" not in self.keys():
            ...
            self["grid_coord"] = torch.div(
                self.coord - self.coord.min(0)[0], self.grid_size, rounding_mode="trunc" # Made self.grid_size to self.grid_size[0]
            ).int()
        ...
        # print("self.grid_coord", self.grid_coord.shape, self.grid_coord[:10])
        # print("code", code.shape, code[:,:10],code.max(), code.min()) # 4xN

        order = torch.argsort(code)  # the order of the code
        ...
        # cdoe torch.Size([4, 521456])
        # order torch.Size([4, 521456])
        # inverse torch.Size([4, 521456])
        # self.grid_coord torch.Size([521456, 3])

        # print("cdoe", code.shape)
        # print("order", order.shape)
        # print("inverse", inverse.shape)
        # print("self.grid_coord", self.grid_coord.shape)
        # raise ValueError("Serialization is not implemented yet.")
