...
def chunking_scene(
    ...
    print(f"Chunking scene {name}")
    dataset_root = Path(dataset_root)
    scene_path = dataset_root  / name
    ...
    for chunk in chunks:
        ...
        if np.sum(mask) < chunk_minimum_size:
            print(f"{name} Chunk size too small, skip,", "coords within the chunk: ", np.sum(mask))
            continue
...
def rename_orig_dirs(chunks_dir, orig_dir):
    name_set = set()

    # Step 1: Collect the first part of each chunk directory name
    for dir_name in os.listdir(chunks_dir):
        full_path = os.path.join(chunks_dir, dir_name)
        if os.path.isdir(full_path):
            name_set.add(dir_name.split("_")[0])

    # Step 2: Rename matching directories in the orig directory
    for name in name_set:
        orig_path = os.path.join(orig_dir, name)
        new_path = os.path.join(orig_dir, f"chunked_{name}")
        if os.path.exists(orig_path) and os.path.isdir(orig_path):
            os.rename(orig_path, new_path)
            print(f"Renamed: {orig_path} -> {new_path}")
        else:
            print(f"Not found or not a directory: {orig_path}")
...
if __name__ == "__main__":
    ...
    data_list = [i for i in os.listdir(config.dataset_root ) if "." not in i]
    ...
    chunk_split_name = (
                f"grid{config.grid_size * 100:.0f}mm_"
                f"chunk{config.chunk_range[0]}x{config.chunk_range[1]}_"
                f"stride{config.chunk_stride[0]}x{config.chunk_stride[1]}")
    chunks_dir = config.dataset_root / chunk_split_name 
    rename_orig_dirs(chunks_dir, config.dataset_root)
