...
class NormalizeGSScale(object):
    def __call__(self, data_dict):
        # TODO get knowledge of scale
        if "scale" in data_dict.keys():
            data_dict["scale"] = data_dict["scale"].clip(0,2) # clip large scale TODO
            data_dict["scale"] = data_dict["scale"] / 2
        return data_dict
...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        for key, value in self.offset_keys.items():
            data[key] = torch.tensor([data_dict[value].shape[0]]) # record the shape of the data
        for name, keys in self.kwargs.items():
            name = name.replace("_keys", "")
            assert isinstance(keys, Sequence)
            # for key_i in keys:
            #     print(key_i, data_dict[key_i].shape)
            data[name] = torch.cat([data_dict[key].float() for key in keys], dim=1)  # feat_keys concat
        return data
...
def quaternion_to_matrix(quaternions: np.ndarray) -> np.ndarray:
    ...
    # Stack components and reshape
    o = np.stack(components, axis=-1)
...
def matrix_to_quaternion(matrix: np.ndarray) -> np.ndarray:
    ...
    flr = 0.1  # Small threshold to avoid division instability
    quat_candidates = quat_by_rijk / (2.0 * np.maximum(q_abs[..., None], flr))
    
    best_idx = np.argmax(q_abs, axis=-1)
    out = quat_candidates[np.arange(best_idx.shape[0]), best_idx]
...
def standardize_quaternion(q):
    """
    Ensure quaternion has a positive real part for consistency.
    """
...
class RandomRotate(object):
    ...
    def __call__(self, data_dict):
        ...
        if "quat" in data_dict.keys():
            quat = data_dict["quat"]
            rot_mat = quaternion_to_matrix(quat) # Gausssian to world
            rot_mat = np.dot(rot_t, rot_mat) # 3x3 @ Nx3x3
...
class GSGaussianBlur(object):
    ...
    def __call__(self, data_dict):
        ...
        if np.random.rand() < self.p:
            ...
            # the coord is already /grid size and unique and sorted
            # if grid_coord is with some order, we can group grid_coord and for each group, we average the color
            # if coord in no order at all, (order come from hash), for efficiency what we can do is Voxelization
            color_grid = np.zeros((*data_dict["grid_size"], data_dict["color"].shape[1])) # Nx3x3
            color_grid[grid_coord[:, 0], grid_coord[:, 1], grid_coord[:, 2]] = data_dict["color"]
            random_sigma = np.random.uniform(self.sigma[0], self.sigma[1])
            color_grid = gaussian_filter(color_grid, sigma=random_sigma)
            data_dict["color"] = color_grid[grid_coord[:, 0], grid_coord[:, 1], grid_coord[:, 2]]
...
class RandomFlip(object):
    ...
    def __call__(self, data_dict):
        ...
        if np.random.rand() < self.p:
            ...
            if 'quat' in data_dict.keys():
                data_dict['quat'][:, 0] = -data_dict['quat'][:, 0] # change the rotation angle
                data_dict['quat'][:, 1] = -data_dict['quat'][:, 1] # change the rotation axis by shift 1
        ...
        if np.random.rand() < self.p:
            ...
            if 'quat' in data_dict.keys():
                data_dict['quat'][:, 2] = -data_dict['quat'][:, 2]
                data_dict['quat'][:, 3] = -data_dict['quat'][:, 3]
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        key = self.hash(grid_coord) # key (739673,)
        # print("grid_coord", grid_coord.shape, "hash", key.shape)
        # what is hash operation fnv_hash_vec, map 3d to 1d hash value
        # understand hash so we know how it argsort
        # NV-1A (Fowler-Noll-Vo) 64-bit hash function, No, the hashed values do not preserve spatial locality in 3D space
        idx_sort = np.argsort(key)
        key_sort = key[idx_sort] # re-order key
        ...
        if self.mode == "train":  # train mode
            ...
            if "sampled_index" in data_dict:
                ...
                # Randomly Selecting One Instance per Unique Hash
                mask = np.zeros_like(data_dict["segment"]).astype(bool)
                mask[data_dict["sampled_index"]] = True
    ...
    def fnv_hash_vec(arr):
        ...
        for j in range(arr.shape[1]):
            hashed_arr *= np.uint64(1099511628211) # x y z, hashed_arr first times
        ...
        if self.mode == "all":
            ...
            if data_dict["coord"].shape[0] > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    if "valid_feat_mask" in data_dict.keys():
                        data_crop_dict["valid_feat_mask"] = data_dict["valid_feat_mask"][idx_crop
                        ]
