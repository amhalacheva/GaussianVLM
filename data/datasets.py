...
GSMIX_REQUIRED_KEYS = [
    ...
    'prompt_middle_1',
    'prompt_middle_2',
    'prompt_after_obj_start',
    'prompt_after_obj_end',
    'feat',
    'coord',
    'grid_size',
    # 'obj_masks',   # this is filled by dataset wrapper
...
class GSBase(Dataset):
    ...
    role_prompt = "You are an AI visual assistant situated in a 3D scene. "\
                  "You can perceive (1) an ego-view image (accessible when necessary), (2) the scene around you (always accessible) and (3) locations, given as bounding boxes <x, y, z, width, height, length> or points <x, y, z> (accessible when necessary). The scene center is at <0, 0, 0>. "\
                  "You should properly respond to the USER's instruction according to the given visual information. "
    situation_prompt = "{situation}"
    egoview_prompt = "Ego-view image:"
    objects_prompt = "" #"Visual information of the scene around you: <scene>"
    task_prompt_start = "USER: {instruction}{location_phrase}" #"</scene> USER: {instruction}{location_phrase}"
    task_prompt_end = "{obj}ASSISTANT:"
    ...
    @staticmethod
    def get_prompts(instruction, situation="", location_phrase="", location="", obj="", dialogue=None):
        if location_phrase!="":
            if obj != "":
                obj += "<obj><placehold><placehold><placehold><placehold></obj>"
            if len(location)==6:
                location_phrase += f" bounding box <loc>{str(location[0])}, {str(location[1])}, {str(location[2])}, {str(location[3])}, {str(location[4])}, {str(location[5])}</loc> "
                #location_phrase_short += f" bounding box <loc>{str(location[0])}, {str(location[1])}, {str(location[2])}, {str(location[3])}, {str(location[4])}, {str(location[5])}</loc> "
            else: 
                location_phrase += f" 3D point <loc>{str(location[0])}, {str(location[1])}, {str(location[2])}</loc> "
        return {
            'prompt_before_obj': GSBase.role_prompt + GSBase.situation_prompt.format(situation=situation),
            'prompt_middle_1': GSBase.egoview_prompt,
            'prompt_middle_2': GSBase.objects_prompt,
            'prompt_after_obj_start': GSBase.task_prompt_start.format(instruction=instruction, location_phrase = location_phrase) if dialogue is None else dialogue,
            #'prompt_after_obj_start_short': GSBase.task_prompt_start.format(instruction=instruction, location_phrase = location_phrase_short) if dialogue is None else dialogue
            'prompt_after_obj_end': GSBase.task_prompt_end.format(obj=obj),
        }
    ...
    def check_output_and_fill_dummy(data_dict):
        ...
        for key in GSMIX_REQUIRED_KEYS:
            if key not in data_dict:
                raise ValueError(f"Key {key} is missing in LeoMix data_dict")
        return data_dict

    def load_rscan_obj_strategy(self, scan_id):
        scan_path = os.path.join(self.rscan_base, '3RScan-ours-align', scan_id)
        pcd_data = torch.load(os.path.join(scan_path, 'pcd-align.pth'))
        points, colors, instance_labels = pcd_data[0], pcd_data[1], pcd_data[2]
        colors = colors / 127.5 - 1
    ...
    def load_scannet_obj_strategy(self, scan_id):
        scan = {}
        pcd_data = torch.load(os.path.join(self.scannet_base, 'scan_data',
                                           'pcd_with_global_alignment', f'{scan_id}.pth'))
        points, colors, instance_labels = pcd_data[0], pcd_data[1], pcd_data[-1]
    ...
    def load_rscan_scene_strategy(self, scan_id):
        chunks = [d for d in os.listdir(self.rscan_base) if scan_id in d]
        feats_all = []
        coord_all = []
        grid_size_all = []
        for chunk in chunks:
            data_dict = {}
            assets = os.listdir(os.path.join(self.rscan_base, chunk))
            for asset in assets:
            ...
            grid_size_all.append(torch.tensor([0.01 for i in range(len(data_dict["coord"]))]))
            coord_all.append(torch.tensor(data_dict["coord"].astype(np.float32)))
        ...
        return {"feat": feats_all, "coord": coord_all, "grid_size": grid_size_all}

        
        # #######################
        # ### TODO 1: check whether color needs normalization
        # #######################
        # points, colors, instance_labels = pcd_data[0], pcd_data[1], pcd_data[2]
        # colors = colors / 127.5 - 1
        # pcds = np.concatenate([points, colors], 1)
    ...
    def load_scannet_scene_strategy_no_center(self, scan_id):
        data_dict = {}
        if self.split =="train":
            data_path = os.path.join(self.scannet_base,self.split, scan_id, self.split, scan_id)
        else:
            data_path = os.path.join(self.scannet_base,"val", scan_id, "val", scan_id)
    ...
    def load_scannet_scene_strategy(self, scan_id):
        ...
        # Clamp in case there are fewer than 40k points
        actual_samples = min(N, num_samples)
        ...
        # Optional: assign to a key
        #data_dict["feat"] = merged.to("cuda")
        return {"feat": merged[indices].to("cpu"), "coord": data_dict["coord"][indices].to("cpu"), "grid_size": data_dict["grid_size"][indices].to("cpu"),"scene_center":data_dict["scene_center"]}

    def preprocess_pcd_scene_strategy(self,  obj_loc, return_anchor=False, situation=None, rot_aug=True):
        # rotate scene
        rot_matrix = None #build_rotate_mat(self.split, rot_aug=rot_aug)
        ...
        if situation is not None:
            ...
            if rot_matrix is None:
                ...
                ori_new = R.from_matrix(ori_new).as_quat()
                ori_new = ori_new.reshape(-1)
            anchor_loc = (pos_new.tolist(), ori_new)
        elif return_anchor:
            anchor_loc = torch.from_numpy(np.array(obj_loc)).float()
        else:
            anchor_loc = torch.zeros(3)
...
@DATASET_REGISTRY.register()
class GSObjSceneCap(GSBase):
    situation_pool = Leo_situation_pool
    instruction_pool = Leo_objcap_instruction_pool
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool

    def __init__(self, cfg, split='train'):
        super().__init__()
        assert split == 'train', "GSObjSceneCap only supports training during the alignment stage"
        self.split = split
        #self.rscan_base = cfg.data.gs_obj_scene_cap.rscan_base
        self.scannet_base = cfg.data.gs_obj_scene_cap.scannet_base
        self.num_points = cfg.data.gs_obj_scene_cap.num_points
        self.max_obj_len = cfg.data.gs_obj_scene_cap.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)

        logger.info("Loading GSObjSceneCap train-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.gs_obj_scene_cap.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSObjSceneCap train-set language, collected {len(self.scan_ids)} data")
        ...
        for fname in os.listdir(anno_dir):
            ...
            if '3rscan' in fname.lower():
                ...
                for item in json_data:
                    ...
                    #all_caps = [item['instance_type']]
                    if "sr3d+" in fname.lower():
                        for c in all_caps:
                            scan_ids.append({
                                'source': 'scannet',
                                'scan_id': item['scan_id'],
                            ...
                            scan_caps.append({
                                'obj_id': item['target_id'],
                                'obj_loc': obj_loc,
                                'caption': ' the'.join(c.split(' the')[1:]),
                                'bbox': bbox,
                            })
                    else:
                        for c in all_caps:
    ...
    def __getitem__(self, index):
        ...
        if scan_id not in self.scan_data[scan_source]:
            if scan_source == '3rscan':
                self.scan_data['3rscan'][scan_id] = self.load_rscan_scene_strategy(scan_id)
            elif scan_source == 'scannet':
                self.scan_data['scannet'][scan_id] = self.load_scannet_scene_strategy(scan_id)
        scan_dict = self.scan_data[scan_source][scan_id]
        # Dict: { int: np.ndarray (N, 6) }

        lang_meta['bbox'] = np.array(lang_meta['bbox'])
        lang_meta['bbox'][:3] -= np.array(scan_dict["scene_center"])
        bbox = np.round(lang_meta['bbox'] , 2)

        obj_loc = np.round(np.array(lang_meta['obj_loc']) - np.array(scan_dict["scene_center"]) , 3)

        anchor_loc = self.preprocess_pcd_scene_strategy(obj_loc, return_anchor=True)
        data_dict = self.get_prompts(
            instruction=random.choice(self.instruction_pool),
            situation=random.choice(self.situation_pool),
            location_phrase=" "+random.choice(self.location_phrase_pool),
            location=bbox,
            obj=random.choice(self.obj_pool),
        )
        data_dict.update({
            'source': scan_source,
            'scene_id': scan_id,
            'feat': scan_dict["feat"].to("cpu"),
            'coord': scan_dict["coord"].to("cpu"),
            'grid_size': scan_dict["grid_size"].to("cpu"),
            'obj_locs': obj_loc.tolist(),
...
@DATASET_REGISTRY.register()
class GSSceneCap(GSBase):
    instruction_pool = Leo_scenecap_instruction_pool

    def __init__(self, cfg, split):
        super().__init__()
        self.split = 'train' if split == 'train' else 'val'
        self.rscan_base = cfg.data.gs_scene_cap.rscan_base
        self.num_points = cfg.data.gs_scene_cap.num_points
        self.max_obj_len = cfg.data.gs_scene_cap.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)

        logger.info(f"Loading GSSceneCap {split}-set language")
        self.scan_ids, self.lang_data, self.scan_insts = self.load_anno(cfg.data.gs_scene_cap.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSSceneCap {split}-set language, collected {len(self.scan_ids)} data")
    ...
    def __getitem__(self, index):
        ...
        # load pcds
        if scan_id not in self.scan_data:
            self.scan_data[scan_id] = self.load_rscan_scene_strategy(scan_id)

        scan_dict = self.scan_data[scan_id]
        ...
        data_dict.update({
            ...
            'scene_id': scan_id,
            'feat': scan_dict["feat"],
            'coord': scan_dict["coord"],
            'grid_size': scan_dict["grid_size"],
            'obj_locs': [-100,-100,-100], #torch.from_numpy(np.array([-1,-1,-1])).float(),
            'img_fts': torch.zeros(3, 224, 224),
            'img_masks': torch.LongTensor([0]).bool(),
            'output_gt': caption,
            "obj_cap": False,
...
class GSScan2Cap(GSBase):
    ...
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_scan2cap.scannet_base
        self.num_points = cfg.data.gs_scan2cap.num_points
        self.max_obj_len = cfg.data.gs_scan2cap.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)
        ...
        if split == 'train':
            ...
            self.split = 'val'
            self.pc_type = "pred" # getattr(cfg.data.gs_scan2cap, 'pc_type', 'gt')

        self.iou_threshold = getattr(cfg.data.gs_scan2cap, 'iou_thres', 0.5)

        logger.info(f"Loading GSScan2Cap {split}-set language")
        self.scan_ids, self.lang_data, self.corpus_cache = self.load_anno(cfg.data.gs_scan2cap.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSScan2Cap {split}-set language, collected {len(self.scan_ids)} data")
    ...
    def __getitem__(self, index):
        ...
        # load pcds
        if scan_id not in self.scan_data:
            self.scan_data[scan_id] = self.load_scannet_scene_strategy(scan_id)
        scan_dict = self.scan_data[scan_id]
...
@DATASET_REGISTRY.register()
class GSSQA3D(GSBase):
    situation_pool = Leo_situation_pool
    your_loc_pool = Your_location_pool
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    ...
    def __init__(self, cfg, split):
        super().__init__()
        self.split = split
        self.scannet_base = cfg.data.gs_sqa3d.scannet_base
        self.num_points = cfg.data.gs_sqa3d.num_points
        self.max_obj_len = cfg.data.gs_sqa3d.max_obj_len
        if split == 'train':
            self.pc_type = 'gt'
        else:
            self.pc_type = "pred" #getattr(cfg.data.gs_sqa3d, 'pc_type', 'gt')
        
        logger.info(f"Loading GSSQA3D {split}-set language")
        self.scan_ids, self.lang_data, self.align_matrices = self.load_anno(cfg.data.gs_sqa3d.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSSQA3D {split}-set language, collected {len(self.scan_ids)} data")
    ...
    def align_situation(self, pos, ori, scene_center, align_matrix):
        ...
        pos_new = pos.reshape(1, 3) @ align_matrix.T
        pos_new -= np.array(scene_center)
        pos_new = pos_new.reshape(-1)
        ori_new=torch.zeros(4)
        ori_new[-1] = 1
        # ori = R.from_quat(ori).as_matrix()
        # ori_new = align_matrix @ ori
        # ori_new = -ori_new   # SQA3D annotation corresponds to the opposite orientation
        # ori_new = R.from_matrix(ori_new).as_quat()
        # ori_new = ori_new.reshape(-1)
        return np.round(pos_new,2), ori_new
    ...
    def __getitem__(self, index):
        ...
        # align situation with pcd
        pos_aligned, rot_aligned = self.align_situation(
            pos, rot, scan_dict['scene_center'], self.align_matrices[scan_id]
        )

        (pos_aligned, rot_aligned) = self.preprocess_pcd_scene_strategy(pos_aligned, return_anchor=False, situation=(pos_aligned, rot_aligned))
        ...
        data_dict = self.get_prompts(
            instruction=self.convert_person_view(question),
            situation=random.choice(self.situation_pool) + ' ' + self.convert_person_view(situation),   
            location_phrase=" "+random.choice(self.your_loc_pool),
            location=pos_aligned
        )
        ...
        data_dict.update({
            ...
            'coord': scan_dict["coord"],
            'grid_size': scan_dict["grid_size"],
            'obj_locs': pos_aligned,
            'situation': situation,
            'anchor_locs': torch.from_numpy(np.array(pos_aligned)).float(),
            'anchor_orientation': rot_aligned,#torch.from_numpy(rot_aligned).float(),
            'img_fts': torch.zeros(3, 224, 224),
            'img_masks': torch.LongTensor([0]).bool(),
            'output_gt': random.choice(answer_list) if self.split == 'train' else answer_list,
            'sqa_type': question_type,
...
@DATASET_REGISTRY.register()
class GSScanQA(GSBase):
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_scanqa.scannet_base
        self.num_points = cfg.data.gs_scanqa.num_points
        self.max_obj_len = cfg.data.gs_scanqa.max_obj_len
        ...
        if split == 'train':
            ...
            self.split = 'val'
            self.pc_type = "pred" #getattr(cfg.data.gs_scanqa, 'pc_type', 'gt')

        logger.info(f"Loading GSScanQA {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.gs_scanqa.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSScanQA {split}-set language, collected {len(self.scan_ids)} data")
...
class GSNr3D(GSScan2Cap):
    ...
    def __init__(self, cfg, split):
        super(GSScan2Cap, self).__init__()
        self.scannet_base = cfg.data.gs_nr3d.scannet_base
        self.num_points = cfg.data.gs_nr3d.num_points
        self.max_obj_len = cfg.data.gs_nr3d.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)
        ...
        self.iou_threshold = getattr(cfg.data.gs_nr3d, 'iou_thres', 0.5)

        logger.info(f"Loading GSNr3D {split}-set language")
        self.scan_ids, self.lang_data, self.corpus_cache = self.load_anno(cfg.data.gs_nr3d.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading GSNr3D {split}-set language, collected {len(self.scan_ids)} data")
...
@DATASET_REGISTRY.register()
class GSSceneCap3DLLM(GSBase):
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_scene_cap_3dllm.scannet_base
        self.num_points = cfg.data.gs_scene_cap_3dllm.num_points
        self.max_obj_len = cfg.data.gs_scene_cap_3dllm.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)
...
@DATASET_REGISTRY.register()
class GSQA3DLLM(GSBase):
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_qa_3dllm.scannet_base
        self.num_points = cfg.data.gs_qa_3dllm.num_points
        self.max_obj_len = cfg.data.gs_qa_3dllm.max_obj_len
        ...
        logger.info(f"Loading LeoQA3DLLM {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.gs_qa_3dllm.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading LeoQA3DLLM {split}-set language, collected {len(self.scan_ids)} data")
...
@DATASET_REGISTRY.register()
class GSPlan3DLLM(GSBase):
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_plan_3dllm.scannet_base
        self.num_points = cfg.data.gs_plan_3dllm.num_points
        self.max_obj_len = cfg.data.gs_plan_3dllm.max_obj_len
        ...
        logger.info(f"Loading LeoPlan3DLLM {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.gs_plan_3dllm.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading LeoPlan3DLLM {split}-set language, collected {len(self.scan_ids)} data")
...
@DATASET_REGISTRY.register()
class GSDialog3DLLM(GSBase):
    location_phrase_pool = Location_instruction_pool
    obj_pool = Obj_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.gs_dialog_3dllm.scannet_base
        self.num_points = cfg.data.gs_dialog_3dllm.num_points
        self.max_obj_len = cfg.data.gs_dialog_3dllm.max_obj_len
        ...
        if split == 'train':
            ...
            self.split = 'val'
            self.pc_type = getattr(cfg.data.gs_dialog_3dllm, 'pc_type', 'gt')

        logger.info(f"Loading LeoDialog3DLLM {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.gs_dialog_3dllm.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading LeoDialog3DLLM {split}-set language, collected {len(self.scan_ids)} data")
...
class GSMix(Dataset):
    mapping = {
        'gs_obj_scene_cap': GSObjSceneCap,
        'gs_scene_cap': GSSceneCap,
        "gs_scan2cap": GSScan2Cap,
        "gs_scanqa": GSScanQA,
        "gs_sqa3d": GSSQA3D,
        'gs_nr3d': GSNr3D,
        'gs_scene_cap_3dllm': GSSceneCap3DLLM,
        'gs_qa_3dllm': GSQA3DLLM,
        'gs_plan_3dllm': GSPlan3DLLM,
        'gs_dialog_3dllm': GSDialog3DLLM,
    }

    def __init__(self, cfg, split):
        self.datasets = []
        self.ratio = cfg.task.leomix.ratio
        logger.info(f"GSMix about to load: {cfg.task.leomix.mix}")
        for dataset in cfg.task.leomix.mix:
            self.datasets.append(self.mapping[dataset](cfg, split))
        ...
        if type(self.ratio) == int or type(self.ratio) == float:
            ...
            self.index_range = list(np.cumsum([int(len(d)*self.ratio[i]) for i, d in enumerate(self.datasets)]))
        self.index_range = [0] + self.index_range
        logger.info(f"Indices of GSMix datasets: {self.index_range}")
    ...
    def streamline_output(data_dict):
        ...
        for key in GSMIX_REQUIRED_KEYS:
            if key not in data_dict:
                raise ValueError(f"Key {key} is missing in GSMix data_dict")
            else:
                new_data_dict[key] = data_dict[key]
        return new_data_dict
